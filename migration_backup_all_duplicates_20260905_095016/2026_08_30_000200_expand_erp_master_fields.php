...
return new class extends Migration
{
    ...
    private function addItems(): void
    {
        ...
        Schema::table('items', function (Blueprint $t) use ($missing) {
            ...
            if (in_array('length', $missing, true)) {
            }
            if (in_array('width', $missing, true)) {
            }
    ...
    private function addCustomers(): void
    {
        ...
        Schema::table('customers', function (Blueprint $t) use ($missing) {
            ...
            if (in_array('fax', $missing, true)) {
                $t->string('fax', 50)->nullable();
            }
            if (in_array('website', $missing, true)) {
                $t->string('website')->nullable();
            }
            if (in_array('billing_postal_code', $missing, true)) {
                $t->string('billing_postal_code', 30)->nullable();
            }
            if (in_array('shipping_postal_code', $missing, true)) {
                $t->string('shipping_postal_code', 30)->nullable();
            }
            if (in_array('currency_code', $missing, true)) {
    ...
    public function down(): void
    {
        $this->dropColumns('items', [
            'short_name','barcode','manufacturer_code','model_no','origin_country',
            'hs_code','warranty_months','weight','height',
            'track_serial','track_batch','purchase_description','sales_description',
        ]);

        $this->dropColumns('customers', [
            'contact_person','mobile',
            'salesperson_code',
            'tax_name','is_pkp','credit_hold','notes',
        ]);

        $this->dropColumns('vendors', [
            'contact_person','mobile','postal_code',
            'lead_time_days','min_order_value','tax_name','is_pkp','purchase_hold','notes',
        ]);

        $this->dropColumns('chart_of_accounts', [
            'account_subcategory','report_group','cash_flow_category','external_code',
            'is_control_account','reconciliation_required','notes',
        ]);
    }
