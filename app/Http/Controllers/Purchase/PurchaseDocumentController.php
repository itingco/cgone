...
use App\Models\{Item, Location, LocationBin, Vendor};
use App\Models\Documents\{PostedReceipt, PurchaseInvoice, PurchaseOrder, PurchaseOrderLine, PurchaseRequest, PurchaseRequestLine, Receipt};
...
class PurchaseDocumentController extends Controller
{
    ...
    public function create(Request $r, string $type)
    {
        ...
        if ($type === 'purchase-invoice' && $r->filled('posted_id')) {
            ...
            if ($src->effectiveStatus() !== 'POSTED') {
                throw new DomainException('Cannot invoice an UNDO Posted Receipt.');
            }
            $record->forceFill([
                'vendor_id' => $src->vendor_id,
                'location_id' => $src->location_id,
    ...
    public function store(Request $r, string $type)
    {
        ...
        $doc = DB::transaction(function () use ($class, $sequence, $data, $type) {
            $h = $data['header'];
            $h['document_no'] = $this->numbers->next($sequence);
            $h['created_by'] = auth()->id();
            $doc = $class::create($h);
            $this->syncLines($doc, $type, $data['lines']);
            $this->audit->record('purchase.'.$type, 'create', $doc, [], $doc->fresh()->toArray(), ['document_number' => $doc->document_no]);
            return $doc;
        });
        return redirect()->route('purchase.documents.show', [$type, $doc->id])->with('success', 'Document created.');
    }
    ...
    public function show(string $type, int $id)
    {
        $this->permit($type, 'view');
        [$class, , $title] = $this->cfg($type);
        $record = $class::with(['lines.item', 'vendor', 'location', 'bin'])->findOrFail($id);
        $progress = [];
        if ($type === 'purchase-order') {
            foreach ($record->lines as $line) {
                ...
                $progress[] = ['item' => $line->item?->code, 'planned' => $planned, 'processed' => $planned - $remain, 'remaining' => $remain, 'label' => 'Received'];
    ...
    public function update(Request $r, string $type, int $id)
    {
        ...
        DB::transaction(function () use ($class, $id, $type, $data) {
            // The row lock must be acquired inside the transaction; otherwise lockForUpdate has no useful lifetime.
            ...
            $doc->lines()->delete();
            $this->syncLines($doc, $type, $data['lines']);
            $this->audit->record('purchase.'.$type, 'update', $doc, $before, $doc->fresh()->toArray(), ['document_number' => $doc->document_no]);
        });

        return redirect()->route('purchase.documents.show', [$type, $id])->with('success', 'Document updated.');
    }
    ...
    public function release(string $type, int $id)
    {
        ...
        DB::transaction(function () use ($class, $id) {
            ...
            foreach ($doc->lines as $line) $this->itemRules->assertEligible($line->item);
    ...
    private function validated(Request $r, string $type): array
    {
        $v = $r->validate([
            ...
            'lines.*.source_purchase_request_line_id' => 'nullable|integer|exists:purchase_request_lines,id',
            'lines.*.source_purchase_order_line_id' => 'nullable|integer|exists:purchase_order_lines,id',
            'lines.*.source_posted_receipt_line_id' => 'nullable|integer|exists:posted_receipt_lines,id',
        ...
        foreach ($v['lines'] as $raw) {
            ...
            if ($item->item_type === 'INVENTORY') {
                if (! $loc) throw new DomainException("Location is required for {$item->code}.");
            ...
            $lineTax = in_array($type, ['purchase-request', 'purchase-order', 'purchase-invoice'], true) ? round($base * $rate / 100, 4) : 0;
            $total = $base + $lineTax;
            $raw = array_merge($raw, [
                'unit_price' => $price,
                'unit_cost' => $cost,
        ...
        if ($type === 'purchase-order') $header['source_purchase_request_id'] = $src;
        if ($type === 'receipt') $header['source_purchase_order_id'] = $src ?: throw new DomainException('Receipt requires a source Purchase Order.');
        if ($type === 'purchase-invoice') $header['source_purchase_order_id'] = $src;
    ...
    private function syncLines(Model $doc, string $type, array $lines): void
    {
        foreach ($lines as $line) {
            if ($type === 'purchase-order' && ! empty($line['source_purchase_request_line_id'])) {
                $src = PurchaseRequestLine::with('document')->findOrFail($line['source_purchase_request_line_id']);
                if ($src->document->status !== 'RELEASED') throw new DomainException('Source Purchase Request must remain RELEASED.');
                if ((int) $src->purchase_request_id !== (int) $doc->source_purchase_request_id || (int) $src->item_id !== (int) $line['item_id']) {
                    throw new DomainException('Purchase Order source line does not match the selected Purchase Request / Item.');
                }
            }

            if ($type === 'receipt') {
                if (empty($line['source_purchase_order_line_id'])) throw new DomainException('Every Receipt line must originate from a Purchase Order line.');
                $src = PurchaseOrderLine::with('document')->findOrFail($line['source_purchase_order_line_id']);
                if ($src->document->status !== 'RELEASED') throw new DomainException('Source Purchase Order must remain RELEASED.');
                if ((int) $src->purchase_order_id !== (int) $doc->source_purchase_order_id || (int) $src->item_id !== (int) $line['item_id']) {
                    throw new DomainException('Receipt source line does not match the selected Purchase Order / Item.');
                }
            ...
            if ($type === 'purchase-invoice' && ! empty($line['source_posted_receipt_line_id'])) {
                $src = \App\Models\Documents\PostedReceiptLine::with('document')->findOrFail($line['source_posted_receipt_line_id']);
                if ($src->document->effectiveStatus() !== 'POSTED') throw new DomainException('Source Posted Receipt has been undone.');
                if ((int) $src->item_id !== (int) $line['item_id'] || (int) $src->document->vendor_id !== (int) $doc->vendor_id) {
                    throw new DomainException('Purchase Invoice source receipt line does not match the Item / Vendor.');
                }
                if ($doc->source_purchase_order_id && $src->source_purchase_order_line_id) {
                    $poLine = PurchaseOrderLine::findOrFail($src->source_purchase_order_line_id);
                    if ((int) $poLine->purchase_order_id !== (int) $doc->source_purchase_order_id) {
                        throw new DomainException('Purchase Invoice source receipt line belongs to a different Purchase Order.');
                    }
                }
    ...
    private function prefill(string $type, Request $r): array
    {
        ...
        if ($type === 'purchase-invoice' && $r->filled('posted_id')) {
            ...
            if ($src->effectiveStatus() !== 'POSTED') throw new DomainException('Cannot invoice an UNDO Posted Receipt.');
            return $src->lines->map(function ($l) {
