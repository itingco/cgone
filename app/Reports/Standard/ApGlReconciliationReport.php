...
final class ApGlReconciliationReport extends AbstractStandardReport implements StandardReport
{
    public function code(): string { return 'AP_GL_RECONCILIATION'; }
    public function parameters(): array { return $this->asOfParameters(); }

    public function run(array $parameters): ReportResult
    {
        $accountIds=app(ControlAccountResolver::class)->payableAccountIds();
        if(!$accountIds){
            return new ReportResult('AP vs GL Reconciliation',$this->columns(),[],[],[],notes:[
                'No Accounts Payable control account is configured in Vendor Posting Group / Vendor master.',
            ],metadata:['configured'=>false]);
        }

        $asOf=CarbonImmutable::parse((string)$parameters['as_of'])->addDay()->startOfDay();

        $sub=DB::table('vendor_ledgers')->where('status','POSTED')->where('posting_at','<',$asOf);
        $this->applyBusinessUnit($sub,'business_unit_id',$parameters);
        $subledger=(float)$sub->selectRaw('COALESCE(SUM(credit-debit),0) AS amount')->value('amount');
        ...
        $glAmount=(float)$gl->selectRaw('COALESCE(SUM(e.credit-e.debit),0) AS amount')->value('amount');
        ...
        $rows=[(object)['source'=>'Accounts Payable','subledger'=>$subledger,'gl'=>$glAmount,'difference'=>$difference,'status'=>$status]];

        return new ReportResult(
            'AP vs GL Reconciliation',$this->columns(),$rows,
