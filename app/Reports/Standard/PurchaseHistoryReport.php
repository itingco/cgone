...
final class PurchaseHistoryReport extends AbstractStandardReport implements StandardReport
{
    public function code(): string { return 'PURCHASE_HISTORY'; }
    public function parameters(): array { return $this->periodParameters(); }

    public function run(array $parameters): ReportResult
    {
        $q=DB::table('posted_purchase_invoices as p')
            ->leftJoin('business_units as bu','bu.id','=','p.business_unit_id')
            ->leftJoin('posted_document_undos as undo', function ($join) {
                $join->on('undo.posted_id','=','p.id')->where('undo.posted_type','=','PostedPurchaseInvoice');
            })
            ->whereNull('undo.id')
        ...
        return new ReportResult(
            ...
            $rows,
            ['total_purchase'=>$this->moneySum($rows,'grand_total')],
