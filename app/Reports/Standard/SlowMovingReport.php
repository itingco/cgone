...
final class SlowMovingReport extends AbstractStandardReport implements StandardReport
{
    public function code(): string { return 'SLOW_MOVING'; }
    ...
    public function run(array $parameters): ReportResult
    {
        ...
        foreach($rows as $row){
            $qty=(float)$row->stock;
            $row->average_cost=abs($qty)>0.0001?(float)$row->stock_value/$qty:0;
        }
        ...
        return new ReportResult(
            ...
            $rows,
            ['slow_moving_items'=>count($rows),'stock_value'=>$this->moneySum($rows,'stock_value')],
